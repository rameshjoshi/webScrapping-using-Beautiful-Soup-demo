# ****************************************************/
# Filename: CS_YP_Batch.py
# Created: Ramesh Joshi
# Purpose: Extract supplier info from Yellow page
# Change history:
...
# print (row[0])
  if row:
    ...
    url_string = "http://www.yellowpages.com/search?search_terms=Concrete+supplier&geo_location_terms=" + row[0] + "%2C+" + row[1]
                  #http://www.yellowpages.com/search?search_terms=electrical+supply&geo_location_terms=boston%2Cma
    print(url_string)
    ...
    for main_div in soup.findAll("div", {"class": "info"})[0:-1]:  # Strip the last element from the list
      ...
      for street_address in main_div.findAll("span", {"itemprop": "streetAddress"}):
        ...
        for url in main_div.findAll("a", {"class": "track-visit-website"}):
          ...
          insert_stmt_sql = 'insert into dbo.tbl_COncreteSupplierlist_web_scrape \
          ...
          cursor.execute ("""insert into dbo.tbl_COncreteSupplierlist_web_scrape(source,city, State,list_no, business_name,Street_address,web_city,Postal_code,phone, business_url ) values (?,?,?,?,?,?,?,?,?,? )""",url_string,row[0],row[1],BusinesslistInt,business_name,street_address,city,postal_code,phone,url)
